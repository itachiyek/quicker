...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/lobby/${open}/detail`, { cache: "no-store" })
      .then((r) => r.json())
      .then(setDetail)
      .catch(() => {});
  }, [open]);
  ...
  return (
    <main className="flex-1 flex flex-col items-center max-w-md w-full mx-auto px-4 pt-5 pb-10 gap-4">
      ...
      <h1 className="w-full text-2xl font-serif font-bold tracking-tight">
        Match history
      </h1>

      {lobbies.length === 0 ? (
        <div className="panel text-sm text-stone-500 text-center p-5 w-full">
          No matches yet. <Link href="/battles" className="underline">Start one</Link>.
        </div>
      ) : (
...
function DetailContent({ detail }: { detail: Detail }) {
  ...
  // Build a question-index → {creator, challenger} map.
...
function Cell({
  ...
  if (!ans)
    return (
      <div className="text-center text-stone-300 tabular-nums">—</div>
    );
  return (
    <div
