...
export default function HistorySheet({
  ...
  const refreshUnclaimed = useCallback(async () => {
    ...
    try {
      ...
      if (r.ok) setUnclaimed(await r.json());
    } catch {
      /* ignore */
    }
  }, [wallet]);
  ...
  useEffect(() => {
    ...
    fetch("/api/lobby/history", { cache: "no-store" })
      ...
      .catch(() => {});
    refreshUnclaimed();
  }, [open, wallet, refreshUnclaimed]);

  const onClaim = async (sym: "WLD" | "USDC") => {
    setClaimError(null);
    if (!unclaimed?.escrow || !unclaimed.tokens) return;
    setClaimingToken(sym);
    const r = await claimFromEscrow({
      escrow: unclaimed.escrow as `0x${string}`,
      token: unclaimed.tokens[sym] as `0x${string}`,
    });
    if (!r.ok) setClaimError(r.reason);
    else await refreshUnclaimed();
  ...
  const wldAmount = Number(unclaimed?.wld ?? 0);
  const usdcAmount = Number(unclaimed?.usdc ?? 0);
  const hasUnclaimed = wldAmount > 0 || usdcAmount > 0;
          ...
          <section className="card-glass w-full p-4 border border-emerald-200/70 bg-emerald-50/60">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="display text-2xl font-black italic tabular-nums text-emerald-900 leading-tight mt-0.5">
